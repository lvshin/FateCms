...
import fate.webapp.blog.resolver.MultipartStream.ItemInputStream;
 ...
 */
public abstract class FCSFileUploadBase {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                if (request != null&&fileName!=null) {
        	        ...
        	        if(list!=null&&list.size()!=0&&names.contains(fileName)){
        	        	int i=0;
        	        	for(;i<list.size();i++){
        	        		if(list.get(i).getFileName().endsWith(fileName))
        	        			break;
        	        	}
        	        	index = i;
        	        }
     ...
     * {@link FCSFileUploadBase#getItemIterator(RequestContext)}.
     */
